...
gdb像是个“年迈而经验丰富的老人”，lldb像是个“与时俱进的活力青年”。就像它们的创造者一样，一个是自由软件的灵魂人物 Richard Stallman，一个是扛起了Apple Swift大旗（及llvm）的Chris Lattner.

[初识LLDB 调试 MySQL](https://zhuanlan.zhihu.com/p/43959058)

[编译器漫谈](https://dashen.tech/2021/04/05/%E7%BC%96%E8%AF%91%E5%99%A8%E6%BC%AB%E8%B0%88/)
...
[Linux工具集快查](https://linuxtools-rst.readthedocs.io/zh_CN/latest/tool/gdb.html)


[初探LLDB调试MYSQL，一条最简单的SELECT语句是如何执行的？](https://www.freesion.com/article/10291373989/)


[[译]使用 LLDB 调试 Go 程序](https://colobu.com/2018/03/12/Debugging-Go-Code-with-LLDB/)
...
```
Unable to find Mach task port for process-id 42060: (os/kern) failure (0x5).
 (please check gdb is codesigned - see taskgated(8))
No symbol table is loaded.  Use the "file" command.
```

按照[此步骤](https://blog.csdn.net/LU_ZHAO/article/details/104803399/)安装证书即可.



[MySQL表自增id用完了该怎么办？](https://baijiahao.baidu.com/s?id=1676252772927806339&wfr=spider&for=pc)

[MySQL只学有用的--你知道自增ID会用完吗？](https://blog.csdn.net/u011296165/article/details/105102377/)

[45 | 自增id用完怎么办？](http://mysql.dashen.tech/mysql45/45.%E8%87%AA%E5%A2%9Eid%E7%94%A8%E5%AE%8C%E6%80%8E%E4%B9%88%E5%8A%9E%EF%BC%9F.html)

[LLDB 调试mysql](https://www.baidu.com/s?ie=utf-8&f=8&rsv_bp=1&srcqid=4697666102400762409&tn=50000021_hao_pg&wd=LLDB%20mysql&oq=LLDB&rsv_pq=c2edcfc500020abe&rsv_t=eb0160a1zSORAOSPyTQ2I8BERcvyPAmYO1y2uiBubUfJ5fRlrY3mD8QcFoH0uaiOniNMo3VK&rqlang=cn&rsv_enter=1&rsv_dl=tb&rsv_sug3=7&rsv_sug1=3&rsv_sug7=100&rsv_sug2=0&rsv_btype=t&inputT=1502&rsv_sug4=1805)
...
### 探究Go程序启动过程(即确定真正的入口函数)

<br>

以下内容是对 雨痕[Go语言学习笔记.下卷](https://book.douban.com/subject/26832468/) 和 [qcrao](https://github.com/qcrao)[Go 语言问题集(Go Questions)--Go 程序启动过程是怎样的](https://www.bookstack.cn/read/qcrao-Go-Questions/%E7%BC%96%E8%AF%91%E5%92%8C%E9%93%BE%E6%8E%A5-Go%20%E7%A8%8B%E5%BA%8F%E5%90%AF%E5%8A%A8%E8%BF%87%E7%A8%8B%E6%98%AF%E6%80%8E%E6%A0%B7%E7%9A%84.md) 实践与记录：
...
<img src="神器gdb/0.png" width = 100% height = 50% />


<img src="神器gdb/1.png" width = 100% height = 50% />
...
查看`/usr/local/go/src/runtime/rt0_linux_amd64.s`
...
<img src="神器gdb/3.png" width = 100% height = 50% />
...
runtime目录下，有很多不同平台的入口文件，均由汇编实现：

<img src="神器gdb/2.png" width = 100% height = 50% />
...
用gdb设置断点，查看`_rt0_amd64`在哪里

<img src="神器gdb/4.png" width = 100% height = 50% />

`src/runtime/asm_amd64.s` 这段汇编就是要找的真正目标~其完成了初始化和运行时的启动:
...
<img src="神器gdb/5.png" width = 100% height = 50% />

<br>

main 函数里执行的一些重要的操作包括：

- 新建一个线程执行 sysmon 函数，定期垃圾回收和调度抢占；
- 启动 gc；
- 执行所有的 init 函数等等
